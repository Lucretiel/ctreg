...
static typing to your regular expressions, allowing syntax errors to be
detected at compile time and capture groups to be matched infallibly.

```
use ctreg::regex;

// Create a regular expression with the macro. This regular expression is
// analyzed at compile time and its normalized representation is emitted as the
// `HelloWorld` type.
...
If the regular expression includes any syntax errors, this will appear as a
compile error, rather than a runtime panic.
```compile_fail
use ctreg::regex;

regex! { HelloWorld = "(?<greeting>Mismatched Parenthesis" };
...
regular expression is analyzed at compile time, and the `$Type` is emitted
containing its normalized representation, with a regex-like API for searching
and matching capture groups. See the [`demo::HelloWorld`] type for an example
...
To keep the API and output types simple, anonymous capture groups are not
present in the capture groups, and are treated identically to non-capturing
groups.

If the regex has no named capture groups, no `captures` method or `Captures`
type is generated.

Because it is not currently possible to create a regular expression in a
`const` context, this macro operates by creating a type instead of an object;
this type's constructor builds a regex at runtime using the post-parse
[normalized form](https://docs.rs/regex-syntax/latest/regex_syntax/hir/struct.Hir.html)
of the expression. In the spirit of 0-cost abstraction, we currently ask the
caller to use their own `OnceLock` or whatever other abstraction is appropriate
to manage the creation and lifespan of this object. This may change in the
future.
*/
#[macro_export]
macro_rules! regex {
...
This type is equivalent to the [`Match`
](https://docs.rs/regex/latest/regex/struct.Match.html) type from the `regex`
crate.
 */
...
It is intended to provide a comprehensive demonstration of the types and
methods generated by [`regex`].
*/
#[cfg(feature = "demo")]
pub mod demo {
    use super::Capture;
    ...
    */
    ...
    impl HelloWorld {
        ...
        */
        ...
        Prefer this method if you only care *that* there was a match, as it
        might be faster than [`find`][HelloWorld::find] or
